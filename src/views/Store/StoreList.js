...
import StoreSetup from "./StoreSetup.js"
...
EnhancedTableToolbar.propTypes = {
 ...
 updateStore = ()=>{
   console.log("Updae button clicked---------")
    ...
    showItemList = () => {
		axios
			.get(process.env.REACT_APP_API + "/store")
			.then((res) => {
    ...
    activeItem = (id, btntext) => {
		...
		axios
			.post(process.env.REACT_APP_API + "/store/changestatus/" + id + "/" + btntext)
			.then((res) => {
				this.setState({
    ...
    onRowDelete = ( id, btntext )=>{
      ...
      axios
      .post(process.env.REACT_APP_API + "/store/changestatus/" + this.state.itemID + "/" + btntext)
      .then((res) => {
    ...
    <GridItem  hidden = {storeListHidden}   xs={12} sm={12} md={12}>
      <Card>
        <CardHeader color="primary">
          <h4 className={classes.cardTitleWhite}>Store List</h4>
          <p className={classes.cardCategoryWhite}>
            Edit, Delete or Approve Store.
          </p>
        </CardHeader>
        <CardBody>
...
{editStoreID && (
<StoreSetup hideStoreEntry={hidStoreInfo} editStoreID={editStoreID}  onclick={this.updateStore}/>
